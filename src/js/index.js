// Zwilla added 2017 some features for a working translation
// please help to translate some of the depending files 'phrases_xxx.js'
// to remember the translation we use now the localstorage
// un-/comment the console.log for developing reasons only
...
DOM.qrImage2 = DOM.qrContainer.find('.qr-image2');
DOM.qrHint = DOM.qrContainer.find('.qr-hint');
DOM.showQrEls = $('[data-show-qr]');
$.mySpanishButton = $('.mySpanishButton');
$.myEnglishButton = $('.myEnglishButton');
$.myFrenchButton = $('.myFrenchButton');
$.myItalianButton = $('.myItalianButton');
$.myJapaneseButton = $('.myJapaneseButton');
$.myCNtradButton = $('.myCNtradButton');
$.myCNsipleButton = $('.myCNsipleButton');

function init() {
    // Events

    // translation events
    $.mySpanishButton.on('click', function () {
        localStorage.setItem('translation', 'spanish');
    ...
    $.myEnglishButton.on('click', function () {
        localStorage.setItem('translation', 'english');
        location.reload(false);
    });

    $.myFrenchButton.on('click', function () {
        localStorage.setItem('translation', 'french');
        location.reload(false);

        $(document).ready(function () {
            localStorage.setItem('translation', 'french');
            translating('french', phrasesFR);
        });
    });

    $.myItalianButton.on('click', function () {

        localStorage.setItem('translation', 'italian');
        location.reload(false);

        $(document).ready(function () {
            localStorage.setItem('translation', 'italian');
            translating('italian', phrasesIT);
        });
    });

    $.myJapaneseButton.on('click', function () {
        localStorage.setItem('translation', 'japanese');
        location.reload(false);

        $(document).ready(function () {
            localStorage.setItem('translation', 'japanese');
            translating('japanese', phrasesJP);
        });
    });

    $.myCNtradButton.on('click', function () {

        localStorage.setItem('translation', 'chinese_traditional');
        location.reload(false);

        $(document).ready(function () {
            localStorage.setItem('translation', 'chinese_traditional');
            translating('chinese_traditional', phrasesCNt);
        });
    });

    $.myCNsipleButton.on('click', function () {

        localStorage.setItem('translation', 'chinese_simplified');
        location.reload(false);

        $(document).ready(function () {
            localStorage.setItem('translation', 'chinese_simplified');
            translating('chinese_simplified', phrasesCNs);
        });
    });
...
function generateClicked() {
    if (isUsingOwnEntropy()) {
        // alert ('You use your own Entropy! We can not generate one. Close Entropy first, thank you!');
        return;
    }
    clearDisplay();
    showPending();
...
function languageChanged() {
    ...
    var currentLang = localStorage.getItem('translation');
    var language = getLanguageFromUrl();


    if (confirm('⚡️All Data 💽 will be cleared 🗑❗ Is that ok❓ ⚡') === true) {
        setTimeout(function () {
            setMnemonicLanguage();
        ...
        alert('Store your datas 💽 next time before doing this❗Thank you️❗')
    }
...
function getDerivationPath() {
    ...
    if (bip49TabSelected()) {
        purpose = parseIntNoNaN(DOM.bip49purpose.val(), 49);
        coin = parseIntNoNaN(DOM.bip49coin.val(), 0);
        account = parseIntNoNaN(DOM.bip49account.val(), 0);
        change = parseIntNoNaN(DOM.bip49change.val(), 0);
        path = 'm/';
        path += purpose + '\'/';
        path += coin + '\'/';
        path += account + '\'/';
        ...
        derivationPath = DOM.bip49path.val();
...
function getLanguage() {
    ...
    var lsLang = localStorage.getItem('translation');
    ...
    try {
        ...
        if (language !== undefined) callback({
            isError: true,
            error: e
        });
    }
    ...
    $(document).ready(function () {
        for (var language in WORDLISTS) {
            if (window.location.hash.indexOf(language) > -1) {
                // console.log('get Language From Url:' + language);
                return language;
            }
        }
        return '';
...
function setMnemonicLanguage() {
    var language = getLanguage();
    // console.log('set Mnemonic Language:' + language);
    ...
    if (!(language in mnemonics)) {
        ...
        // console.log('New Mnemonic:' + language);
    }
    mnemonic = mnemonics[language];
    // console.log('is Mnemonic:' + language);
}
...
function setMnemonicFromEntropy() {
    ...
    // console.log('entropyMnemonicLength: ' + mnemonicLength);
    ...
    // console.log('setMnemonicFromEntropy -> phrase:' + phrase);
...
function createQr(e) {
    ...
    // console.log('ecl: ' + ecl);
    var segs = qrcodegen.QrSegment.makeSegments(content);
...
$(document).ready(function () {
    ...
    if (localStorage.getItem('translation')) {
        // alert('on load into:' + localStorage.getItem('translation'))
        translating(localStorage.getItem('translation'));
    } else {
        localStorage.setItem('translation', 'english')
    }
});
...
// (@zwilla) added this function 2017, to make sure all datas are wiped after unload!!!
window.addEventListener('unload', function (e) {
    ...
    // localSorage only used to store the translation
    // localStorage.clear();
    console.clear();
    // alert('All wiped!');
    // window.close();
